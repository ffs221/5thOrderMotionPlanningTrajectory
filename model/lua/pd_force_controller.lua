function pd_force_controller(K,D,inData)
    
    -- 1. Proportional part:
    local ctrl=K*(inData.targetPos - inData.currentPos)
    
    -- 2. Derivative part:
    if not inData.first then
        local currentVelocity = (inData.currentPos - lastCurrentPosition)/inData.dynStepSize
        ctrl=ctrl+D*(inData.targetVel - currentVelocity)
    end
    lastCurrentPosition=inData.currentPos
    
    -- 3. Feedforward Force Part
    ctrl=ctrl+inData.maxForce
    
    -- 3. Calculate the max velocity and absolute force
    local desiredVelocity = 0.
    local desiredForce = 0.
    if ctrl > 0 then
        desiredVelocity = inData.velUpperLimit
        desiredForce = ctrl
    else
        desiredVelocity = -inData.velUpperLimit
        desiredForce = -ctrl
    ...
    firstPass=false
    local outData={}
    outData.velocity=desiredVelocity
    outData.force=desiredForce
    return outData
    ...
    -- inData.passCnt : the current dynamics calculation pass. 0-9 by default. See next item for details.
